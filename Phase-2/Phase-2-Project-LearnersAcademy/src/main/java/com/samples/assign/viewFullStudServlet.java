...
@WebServlet("/viewFullStudServlet")
public class viewFullStudServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try (Statement statement = connection.createStatement();) {
			...
			ResultSet results = statement.executeQuery("select * from MasterStudents");
			PrintWriter out = response.getWriter();
			
			out.println("<h1>Students assinged...</h1>");
			out.println("<table>");
			out.println("<tr>");
